...
export class QuestionOptionsController {
  ...
  postAdd(@UploadedFile() profile: Express.Multer.File, @Body() createquestion: option_dto): object {
    new ParseFilePipe({
      validators: [
  ...
  @Patch(':id')
  async update(@Param('id') id: string, @Body() updateparticipants: option_dto) {
