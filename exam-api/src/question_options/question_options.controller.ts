...
export class QuestionOptionsController {
  ...
  postAdd(
    ...
    @Body() createquestion: option_dto,
  ): object {
    new ParseFilePipe({
      validators: [
    ...
    const create_question = this.questionoption.create(
      createquestion,
      profile.path,
    );
  ...
  async update(
    @Param('id') id: string,
    @Body() updateparticipants: option_dto,
  ) {
    const update_participants = await this.questionoption.update(
      id,
      updateparticipants,
    );
